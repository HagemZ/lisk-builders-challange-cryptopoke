...
type Props = {
  params: Promise<{ moonsterName: string }>;
...
export async function generateMetadata({ params }: Props) {
  ...
  const moonsterName = decodeURIComponent(resolvedParams.moonsterName);
  return {
    title: `Moonster - ${moonsterName.charAt(0).toUpperCase() + moonsterName.slice(1)} Joined Round`,
    description: `Battle round join result for ${moonsterName} in Moonster.`,
  };
}

export default async function JoinResultPage({ params, searchParams }: Props) {
  const resolvedParams = await Promise.resolve(params);
  ...
  // Handle the error parameter with safe decoding
  ...
  if (error) {
    ...
    try {
      ...
      console.error('Failed to decode error message:', err);
      errorMessage = 'Invalid error message received. Please try again.';
    }
  }
        ...
        <div className="relative w-full mx-auto p-4">
          ...
          <div className="relative p-4 mt-4">
            <div className="bg-gray-800 rounded-md shadow-xl p-6 border border-neutral-800 text-center">
              {errorMessage ? (
                <>
                  <h1 className="text-2xl font-bold text-gray-50 mb-4">Failed to Join Round</h1>
                  <p className="text-gray-300 mb-4">
                    Your Moonster <span className="font-semibold capitalize">{moonster.name}</span> could not join the battle round: {errorMessage}.
                  </p>
                </>
              ) : (
                <>
                  <h1 className="text-2xl font-bold text-gray-50 mb-4">Join Successful!</h1>
                  <p className="text-gray-300 mb-4">
                    Congratulations! Your Moonster <span className="font-semibold capitalize">{moonster.name}</span> has joined the battle round.
                  </p>
                </>
              )}
